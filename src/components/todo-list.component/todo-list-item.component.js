...
export default function TodoListItem(props) {
  ...
  const removeOne = (e, todoToDelete) => {
    e.preventDefault();
    const todos = props.todosState.todosArr.filter((todo) => todo.id !== todoToDelete.id)
    props.setTodosState({...props.todosState, todosArr: todos}) 
  }
  const completeOne = (e, todoToComplete) => {
    e.preventDefault();
    const todos = props.todosState.todosArr.filter((todo) => todo.id !== todoToComplete.id)
    props.setTodosState({...props.todosState, todosArr: todos, completedTodos: [...props.todosState.completedTodos, todoToComplete]}) 
  }
  return (
    <ListItem sx={{p: 0 }}>
